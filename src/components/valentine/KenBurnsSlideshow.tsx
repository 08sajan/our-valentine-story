...
export const KenBurnsSlideshow = ({ 
  ...
  const SlideshowContent = ({ fullscreen = false }: { fullscreen?: boolean }) => (
    <div 
      ...
      onMouseEnter={() => !fullscreen && setShowControls(true)}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <motion.img
            ...
            className={`${fullscreen ? 'max-h-full max-w-full' : 'w-full h-full'} object-contain`}
            initial={{ 
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="bg-gradient-to-t from-black/80 via-black/50 to-transparent pt-16 pb-6 px-6">
            <motion.p 
              className={`text-white text-center font-serif ${fullscreen ? 'text-2xl' : 'text-lg'}`}
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <motion.button
                ...
                className="p-4 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/20 hover:bg-black/60 transition-all"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
              >
                {isPlaying ? (
            ...
            <motion.button
              onClick={prevSlide}
              className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/20 hover:bg-black/60 transition-all"
            ...
            <motion.button
              onClick={nextSlide}
              className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/40 backdrop-blur-md text-white border border-white/20 hover:bg-black/60 transition-all"
              whileHover={{ scale: 1.1, x: 3 }}
              whileTap={{ scale: 0.9 }}
            >
